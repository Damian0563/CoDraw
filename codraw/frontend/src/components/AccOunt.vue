	...
	<main class="main-layout">
		...
		<div class="container flex-grow-1 d-flex flex-column mt-4 mt-md-5 align-items-start">
			...
			<div class="boards-wrapper mt-2 justify-content-start">
				<div v-for="(board, index) in boards" :key="index" class="project-card">
					<div class="d-flex justify-content-end align-items-center gap-2">
						<div v-if="admin" class="w-100">
							<div class="d-flex justify-content-end align-items-center gap-2">
								<button class="text-start btn btn-primary"
									@click="exportImg(previewing[board.room], board.title)">Export</button>
								<button class="btn btn-success success" @click="join(board.room)">
									Join room
								</button>
								<img v-if="!edits[index]" :src="edit" loading="lazy" decoding="async" alt="edit"
									@click="edits[index] = !edits[index]" style="width:20px;height:20px;cursor:pointer;">
								<img v-else :src="save" loading="lazy" decoding="async" alt="save"
									@click="edits[index] = !edits[index]; resave(boards[index]); boards[index].modified = 'Just now'"
									style="width:20px;height:20px;cursor:pointer;">
								<img :src="bin" loading="lazy" alt="delete" decoding="async"
			...
			<div class="container mt-2" v-if="admin">
				...
				<div class="boards-wrapper mt-5 justify-content-start">
					...
					<div v-if="bookmarks.length === 0" class="text-center mb-3 fw-semibold"
